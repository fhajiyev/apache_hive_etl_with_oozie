...
-- SKN - 그랜드워커힐
  ...
  INSERT OVERWRITE TABLE dmp.prod_data_source_store PARTITION (part_hour='${hivevar:hourbefore}', data_source_id='114')
  SELECT
  a.DMP_UID as uid,
  a.REQUEST_DATE,
  a.REQUEST_TIME,
  ...
  if(ACTION IN ('search','reservation'), HOTELS, ''),
  ...
  if(ACTION IN ('store_view','store_basket','store_purchase'), PROD_NM, ''),
  if(ACTION IN ('store_view','store_basket','store_purchase'), REGEXP_REPLACE(PROD_PR, ',', ''), ''),
  ...
  SELECT
        ...
        reflect('java.net.URLDecoder', 'decode', if(get_json_object(BODY, '$.hotels') is null, '', get_json_object(BODY, '$.hotels'))) AS HOTELS,
        ...
        reflect('java.net.URLDecoder', 'decode', if(get_json_object(BODY, '$.product_name') is null, '', get_json_object(BODY, '$.product_name'))) AS PROD_NM,

        reflect('java.net.URLDecoder', 'decode', if(get_json_object(BODY, '$.product_price') is null, '', get_json_object(BODY, '$.product_price'))) AS PROD_PR,
  ...
  AND DSID IS NOT NULL AND DSID = '1025'
