...
insert overwrite table dmp_pi.solution_pre_mkt partition (part_date='${hivevar:day2before}')
...
insert overwrite table dmp_pi.solution_pre_mkt_all partition (part_date='${hivevar:day2before}')
  select
...
--    tt.mname,
    ...
    tt.serv_sol_id,
    tt.mkt_nm,
    if(sa.seg_id is null, '', sa.seg_id),
    if(sa.seg_nm is null, '', sa.seg_nm)

  from
  (              
    select mkt_id, mbr_id, max(std_tm) tm, poc_ind_cd, dt, sltn_full_actn_cd, sid, '' as seg_id, '' as seg_nm
  ...
  FROM
        ...
        -- 수신/반응건수
            ...
            WHERE PART_DT = '${hivevar:day2before}'  -- 수신, 반응 일자 겸 PARTITION 날짜
 ...
 JOIN
        (
            SELECT
                     ...
                     ,CASE WHEN PUSH_ID IS NULL THEN '0'
                      ...
                      END AS PUSH_ID
            FROM SMARTWALLET.PROC_SYW_D_PUSH_LOG
            WHERE PART_DT = '${hivevar:day2before}'  -- 수신 일자 겸 PARTITION 날짜
  ...
  left outer join
  (
    select mkt_id, taid, aname, lc, mc, sc, lm, mm, sm, serv_sol_id, mkt_nm from dmp_pi.solution_pre_mkt where part_date='${hivevar:day2before}'
--    select mkt_id, taid, aname, lc, mc, sc, lm, mm, sm, sid, mid, mname, sid_name from dmp_pi.solution_pre_mkt where part_date='${hivevar:day2before}'
  ) tt 
...
-- insert overwrite table dmp_pi.prod_data_source_store partition (part_hour='${hivevar:day2before}00', data_source_id='7')
-- select
...
-- where att.part_date = '${hivevar:day2before}' and att.action not in ('06', '07')



-- alter table dmp_pi.prod_data_source_store drop partition (part_hour < '${hivevar:day367before}00', data_source_id='7')
-- alter table dmp_pi.prod_data_source_store_parquet drop partition (part_hour < '${hivevar:day367before}00', data_source_id='7')
