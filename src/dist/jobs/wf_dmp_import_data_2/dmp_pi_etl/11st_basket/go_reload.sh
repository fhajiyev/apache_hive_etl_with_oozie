...
if [ "$TDY" == "20180217" ]; then
LAST_JOB_DAY=`expr 2 + $1`
LAST_JOB_DAY_270_AGO=`expr 271 + $1`
elif [ "$TDY" == "20180218" ]; then
LAST_JOB_DAY=`expr 3 + $1`
LAST_JOB_DAY_270_AGO=`expr 272 + $1`
elif [ "$TDY" == "20180219" ]; then
LAST_JOB_DAY=`expr 4 + $1`
LAST_JOB_DAY_270_AGO=`expr 273 + $1`
fi
...
insert overwrite table dmp_pi.prod_data_source_store_alter partition (part_hour='${LAST_JOB_DATE_NOSLA}00', data_source_id='17')
...
run /app/di/script/run_hivetl.sh -e "alter table dmp_pi.prod_data_source_store_alter drop partition (part_hour < '${DROP_DATE}00', data_source_id='17');"
