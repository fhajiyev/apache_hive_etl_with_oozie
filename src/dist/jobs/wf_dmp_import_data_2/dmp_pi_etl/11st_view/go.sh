...
etl_stat 'DOING'
run cp $DMP_PI_LOCAL_HOME/11st_disp_ct_dic.dat $DMP_PI_LOCAL_HOME/11st_view/
...
insert overwrite table dmp_pi.prod_data_source_store partition (part_hour='${LAST_JOB_DATE_NOSLA}00', data_source_id='15')
...
select
 ...
 from
 11st.tb_evs_ods_f_pd_prd_hits A
 join
 ...
 where A.part_date = '${TODAY_JOB_DATE}'
   ...
   and regexp_replace(substr(A.hits_dt,1,10),'-','') = '${LAST_JOB_DATE_NOSLA}'   
...
;" > 11st_view_${LAST_JOB_DATE_NOSLA}.hql


run /app/di/script/run_hivetl.sh -f 11st_view_${LAST_JOB_DATE_NOSLA}.hql

run /app/di/script/run_hivetl.sh -e "alter table dmp_pi.prod_data_source_store drop partition (part_hour < '${DROP_DATE}00', data_source_id='15');"

etl_stat 'DONE'

rm -f 11st_view_${LAST_JOB_DATE_NOSLA}.hql
