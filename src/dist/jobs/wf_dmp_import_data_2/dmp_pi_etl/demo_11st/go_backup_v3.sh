   ...
   (
      ...
      (
         ...
         (
            ...
            buy_mem_no_de,
            delvplace_seq,
            ROW_NUMBER() OVER( PARTITION BY buy_mem_no_de ORDER BY ord_no DESC ) as rn

            from
            11st.tb_evs_ods_f_tr_ord_prd
            where
            part_date = '${TODAY_JOB_DATE}' and buy_mem_no_de not in ('','-1','0')
         ) a1
         where a1.rn = 1
      )
      DD
      ...
      EE
      on
      DD.delvplace_seq = EE.delvplace_seq
