...
insert overwrite table dmp_pi.prod_data_source_store partition (part_hour='${LAST_JOB_DATE_NOSLA}00', data_source_id='11')


select distinct uid, col001, col002, col003, col004,
'','','','','','',
'','','','','','','','','','',
'','','','','','','','','','',
'','','','','','','','','','',
...
select ma.ci_no as uid, ma.card_gubun as col001, ma.card_dtl_grp_cd as col002, ma.dtl_cd_nm as col003, ma.card_iss_dt as col004
...
select  d.ci_no, a.mbr_id, a.card_sts_cd, a.card_dtl_grp_cd, a.iss_alcmpn_cd, a.card_iss_dt, 
  ...
  join (select dtl_cd , dtl_cd_nm 
   ...
   where d.ci_no is not null and d.ci_no <> ''
) ma
union all 
select mb.ci as uid, mb.card_gubn as col001, mb.card_id as col002, mb.card_name as col003, mb.issue_dt as col004
from (
select 
...
" > demo_cards_${LAST_JOB_DATE_NOSLA}.hql


run /app/di/script/run_hivetl.sh -f demo_cards_${LAST_JOB_DATE_NOSLA}.hql

run /app/di/script/run_hivetl.sh -e "alter table dmp_pi.prod_data_source_store drop partition (part_hour < '${DROP_DATE}00', data_source_id='11');"

etl_stat 'DONE'

rm -f demo_cards_*.hql
