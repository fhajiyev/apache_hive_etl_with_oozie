...
insert overwrite table dmp_pi.prod_data_source_store_alter partition (part_hour='${LAST_JOB_DATE_NOSLA}00', data_source_id='4')
...
ocb.mart_mbr_mst
where
ci_no IS NOT NULL AND ci_no <> '' AND ci_no not like '%\u0001%' AND mbr_sts_cd = 'A'
...
ocb.log_touch_50
where
part_hour between '${LAST_JOB_DATE_NOSLA}00' and '${LAST_JOB_DATE_NOSLA}24'
and
...
run /app/di/script/run_hivetl.sh -e "alter table dmp_pi.prod_data_source_store_alter drop partition (part_hour < '${DROP_DATE}00', data_source_id='4');"

etl_stat 'DONE'

rm -f ocb_act_${LAST_JOB_DATE_NOSLA}.hql
