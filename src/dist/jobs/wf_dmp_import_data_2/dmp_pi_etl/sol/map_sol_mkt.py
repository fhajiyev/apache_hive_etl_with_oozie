...
#18  svc_sltn_id
    ...
    for line in sys.stdin:
        ...
        if sltn_full_actn_cd == '0107EN01':  # 두툼 적립
            action = '01'
        elif sltn_full_actn_cd == '0107PU01': # 두툼 사용
            action = '02'
        elif sltn_full_actn_cd == '0107PU02': # 두툼 취소
            action = '03'
        elif sltn_full_actn_cd == '0104PU01': # 무료쿠폰 사용
            action = '04'
        else:
            continue


        if poc_ind_cd == 'O':            # OCB면
            mbr_id = 'OCB_' + mbr_id
	    channel = '01'

        elif poc_ind_cd =='S':          # 시럽이면
            mbr_id = 'SYRUP_' + mbr_id
	    channel = '02'
        else:
            continue
        #                            발생 시각, sid = 오시비 가맹점 코드, # 서비스 솔루션 ID
