...
HIVEDIR1=hdfs://skpds/user/dmp_pi/dmp_pi_store_db_alter/part_hour=${TARGET_JOB_DATE}00
HIVEDIR2=$HIVEDIR1/data_source_id=8
$HDD fs -mkdir $HIVEDIR1
run $HDD fs -mv $OUT_ETL_ZONE $HIVEDIR2

run /app/di/script/run_hivetl.sh -e 'MSCK REPAIR TABLE dmp_pi.prod_data_source_store_alter;'
run /app/di/script/run_hivetl.sh -e "alter table dmp_pi.prod_data_source_store_alter drop partition (part_hour < '${DROP_DATE}00', data_source_id='8');"
etl_stat 'DONE'
