...
CREATE EXTERNAL TABLE user_dmp.mahout_syrupad_output_${oozie_job_name} ( duid_sq bigint, value MAP<string, string> ) CLUSTERED BY (duid_sq) SORTED BY (duid_sq) INTO 256 BUCKETS ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t' ESCAPED BY '[' COLLECTION ITEMS TERMINATED BY ',' MAP KEYS TERMINATED BY ':' LINES TERMINATED BY '\n' STORED AS TEXTFILE LOCATION 'hdfs://skpds/${syrupad_pbp_hdfs_home}/${oozie_job_name}/mahout_output/recommendations';
