...
-- Make Service Table Data from Mahout output

INSERT INTO TABLE svc_ds_dmp.tmp_syrupad_recommend_${oozie_job_name} SELECT FROM_UNIXTIME(UNIX_TIMESTAMP(), 'yyyy/MM/dd HH:mm'), b.d_uid, c.m_slot, c.client_type, a.ads,   a.score, '${jobId}', FROM_UNIXTIME(UNIX_TIMESTAMP(), 'yyyyMMdd') FROM ( SELECT duid_sq, ads,   REGEXP_REPLACE(score, '^\\[|\\]$', '') AS score FROM user_dmp.mahout_syrupad_output_${oozie_job_name} LATERAL VIEW EXPLODE(value) map_value_table AS ads, score WHERE ((ads <> 'sample'))) a JOIN user_dmp.mahout_duid_duid_sq_mapping_${oozie_job_name} b ON b.duid_sq = a.duid_sq JOIN user_dmp.slot_ad_${oozie_job_name} c ON c.ads = a.ads
